...
class MLP :
    def __init__(self,N=100,L=1,K=2, N_real=1,tolerance=0.1,decision=10,maxit=500,rseed=0,out_adjacency=False,inf=1e10,err_max=0.00001,err=0.1,initialization=0,undirected=False,folder="data/",end_file="",adj="SocialMedia.txt",w_file="w.txt",assortative=False):	
        self.N=N
        self.L=L
        self.K=K
        self.N_real=N_real
        ...
        for i in range(self.L):
            for k in range(self.K):
                if(self.assortative==True):
                    self.w[k,i]=rng.random_sample(1)
                else:
            ...
            if(nr>0):
                ...
                for k in range(self.K):
                    if(assortative==False):
                        self.w[k][k][l]=float(a[k+1])
                    else:
                        self.w[k][l]=float(a[k+1])
        infile.close()
        for l in range(self.L):
            for k in range(self.K):
                if(self.assortative==True):
                    self.w[k][l]+=self.err*rng.random_sample(1)
                else:
    ...
    def _update_old_variables(self,u_list,v_list):
        ...
        for i in range(len(v_list)):
            for k in range(self.K):
                self.v_old[v_list[i]][k]=self.v[v_list[i]][k]
        for l in range(self.L):
            for k in range(self.K):
                if(self.assortative==True):
                    self.w_old[k][l]=self.w[k][l]
                else:
                    for q in range(self.K):
